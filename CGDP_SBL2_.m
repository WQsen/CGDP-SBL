...
%%
tstart1 = tic;
[Source_power_re,Azimuth,source_id_,j1]=CGDP_SBL2(Y,Azimuth_grid,grid_interval,Aa,K);   % K is known
...
legend('CGDP-SBL')
function [source_power,Azimuth,source_id,iter]=CGDP_SBL2(Y,Azimuth_grid,grid_interval,Aa,K_)
%% input:
%% Y: Array output data
...
%  KP = min(length(PeakLoc0),M-1);
   KP = min(length(PeakLoc0),K_);
   SigLoc = PeakLoc0(SortIdx(1:KP));
   source_id=sort(SigLoc,'ascend');
   Ac=[Aa(:,source_id)]; 
   P=[]; 
   P=Ac*(Ac'*Ac)^(-1)*Ac';                           
   Beta=1/abs(trace((eye(M)-P)*Ry)/(M-KP));% eq.(18)
