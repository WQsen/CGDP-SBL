...
%%
tstart1 = tic;
[Source_power_re,Azimuth,source_id_,j1]=CGDP_SBL2(Y,Azimuth_grid,grid_interval,Aa);   
time1=toc(tstart1);
...
legend('CGDP-SBL')
function [source_power,Azimuth,source_id,iter]=CGDP_SBL2(Y,Azimuth_grid,grid_interval,Aa)
%% input:
%% Y: Array output data
...
%% CGDP-SBL2:
  ...
  [~,SortIdx] = sort(source_power(PeakLoc0),'descend');%descend the spectrum£¬SortIdx 
   ...
   spd=spd_(1:M-1); [spd,~]=sort(spd,'ascend'); 
   signal_id=intersect(source_id_,spd); % signal index
