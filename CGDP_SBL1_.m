...
phase_noise=1/4*(randn(M,K)+1j*randn(M,K));
% Y=(A+phase_noise)*S+noise;
Y=A*S+noise;
...
%%
tstart1 = tic;
[Source_power_re,Azimuth,source_id_,j1]=CGDP_SBL1(Y,Azimuth_grid,grid_interval,Aa);   
time1=toc(tstart1);
...
legend('CGDP-SBL1')
function [source_power,Azimuth,source_id,iter]=CGDP_SBL1(Y,Azimuth_grid,grid_interval,Aa)
%% input:
%% Y: Array output data
...
jmax=500;
xi=ones(N,1);
% xi=10^(-2)*ones(N,1); % or a small value such as, xi=10^(-2)*ones(N,1).
iter=0;
sigma_y=[];
gamma=[];
tol=0.001; % tolorance
...
%% CGDP-SBL1
   ...
   Beta=(M*L)/(((norm(Y-Aa*Mu_x,'fro'))^2)+L*trace(B-B*sigma_y_inv*B));  % eq.(17)
